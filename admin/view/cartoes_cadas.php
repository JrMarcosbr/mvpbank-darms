        ...
        <div class="card my-4">
          <div class="p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-primary  pt-4 pb-3">
              <h2 class="text-white text-capitalize text-center ps-3">Aprovação de Cadastro do Cartão</h2>
            </div>
          </div>
                ...
                if ($row_cli > 0) {
                  while ($value = mysqli_fetch_assoc($sql)) {
                    ...
                    echo "<td>".$value['data_validade']." - ".$value['tempo_validade']." anos</td>";
                     echo '<td><button type="button" class="btn btn-info" data-toggle="modal" data-target="#exampleModal-'.$value["id_cartao"].'">☰</button></td>';
                  }
                }else{
