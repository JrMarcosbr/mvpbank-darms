        ...
        <div class="card my-4">
          <div class="p-0 position-relative mt-n4 mx-3 z-index-2">
            <div class="bg-gradient-primary  pt-4 pb-3">
              <h2 class="text-white text-capitalize text-center ps-3">Gerenciamento de Clientes</h2>
            </div>
          </div>
          <div class="table-responsive col-md-12 mt-4  mx-auto">
            <table id="exampla"  class="table text-center">
              ...
              <tbody class="text-center">
                <?php
                $query = "SELECT *,count(id_user) as qtd_cartao FROM `tb_cliente` LEFT OUTER JOIN tb_cartao on tb_cartao.tb_cartao_id_user = tb_cliente.id_user GROUP BY id_user";
                $sql = mysqli_query($con, $query);
                $row_cli = mysqli_num_rows($sql);
                if ($row_cli > 0) {
                  while ($value = mysqli_fetch_assoc($sql)) {
                    echo "<tr>";
                    echo "<td>00".$value['id_user'] ."</td>";
                    echo "<td>".$value['nome_user'] ."</td>";
                    echo "<td>".$value['email_user'] ."</td>";
                    echo "<td>".$value['cpf_user'] ."</td>";
                    echo "<td>".$value['contato_user'] ."</td>";
                    ...
                    if ($value['qtd_cartao'] != 0) {
                    	$qtd = $value['qtd_cartao'];
                    }else{
                    	$qtd = "0";
                    }
                    echo "<td>".$qtd."</td>";
                    // echo "<td><a class='btn btn-info text-white' href='edit_produto.php?id_user=".$value['id_user']."'>Editar</a></td><td><button class='btn btn-danger' onclick=excluirItemSelecionado(".$value['id_user'].")>Excluir</button></td>";
                    echo "</tr>";
                  }
                }else{
